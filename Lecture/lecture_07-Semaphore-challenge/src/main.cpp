...
static SemaphoreHandle_t buf_mut;			// Mutex to protect the buffer during access
static SemaphoreHandle_t ser_mut;			// Mutex to protect printing resource
...
void consumer(void *parameters) {
	...
	while (1) {
		...
		if (xSemaphoreTake(buf_mut, 0) == pdTRUE) {
			val = buf[tail];
			tail = (tail + 1) % BUF_SIZE;
		}
		// TODO: Implement mutex to protect serial print resources
		// if (xSemaphoreTake( ser_mut, 0) == pdTRUE) Serial.println(val);
		// xSemaphoreGive(ser_mut);
		Serial.println(val);
...
void setup() {
	...
	ser_mut = xSemaphoreCreateMutex();
	buf_mut = xSemaphoreCreateMutex();
